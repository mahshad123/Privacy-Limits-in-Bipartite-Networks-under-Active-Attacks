%% Effect of Query respond noise on the number of queries
...
P1=0.01;   % first probability of  error for query matrix
P2=0.3;   % Second probability of  error for query matrix
...
for(t=1:matrixgen)
    ...
    for(m= [1000 2000 5000 10000])
        ...
        r=0.4*m;
        mu=100*r;
        [av_c av_I]=Nq(m,r,mu,ITER,alpha,P1,P2,k)
        A(k,t,tt)=av_c;
        B(k,t,tt)=av_I;
    end
end
...
legend({'theta=1','theta=2','theta=3','theta=4','theta=5'},'Location','southeast')
xlabel('Users') 
ylabel('1-Pe') 
figure
